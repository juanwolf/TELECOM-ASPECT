...
public aspect SimulationTracing {
	...
	after(Object x) : Pointcuts.customerHangUp() && target(x) {
		addToLogAfterMessage(thisJoinPoint, x);
	}
	
	after(Object x) : Pointcuts.customerPickUp() && target(x) {
		addToLogAfterMessage(thisJoinPoint, x);
	}
	
	after(Object x) : Pointcuts.callInvite() && target(x) {
		addToLogAfterMessage(thisJoinPoint, x);
	}
